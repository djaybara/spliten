...
export function shuffleArray<T>(arr: T[], seed = 1337): T[] {
  let a = 1664525, c = 1013904223, state = seed >>> 0;
  const rand = () => { state = (Math.imul(a, state) + c) >>> 0; return state / 0xffffffff; };
...
export function preventTriples<T>(items: T[], key: (x: T) => string): T[] {
  ...
  for (const it of items) {
    const k = key(it), n = out.length;
    if (n >= 2 && key(out[n - 1]) === k && key(out[n - 2]) === k) {
      let swapped = false;
      for (let s = n - 1; s >= 0; s--) {
        if (key(out[s]) !== k) { out.splice(s + 1, 0, it); swapped = true; break; }
      }
